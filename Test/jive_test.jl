...
function make_data(; seed = 2024, noise = 0.3)
    ...
    U1 = randn(p1, rT); U2 = randn(p2, rT)     # block-specific joint loadings
    S1 = randn(r1T, n); W1 = randn(p1, r1T)    # block 1's individual signal
    S2 = randn(r2T, n); W2 = randn(p2, r2T)    # block 2's individual signal
...
@testset "ground truth: recovers joint subspace (noiseless)" begin
    ...
    X1c = U1*S + W1*S1; X2c = U2*S + W2*S2                # clean, noiseless data
...
@testset "argument validation" begin
    ...
    Random.seed!(0)
    X1 = randn(20, 50); X2 = randn(15, 40)               # 50 vs 40 columns — incompatible
...
@testset "matches r.jive (offline reference fixtures, given ranks)" begin
    ...
    if !isfile(joinpath(refdir, "X1.csv"))
        ...
        D1 = rd("D1.csv"); D2 = rd("D2.csv")              # r.jive's preprocessed (scaled) blocks
        J1 = rd("J1.csv"); J2 = rd("J2.csv")              # r.jive's joint structure
        A1 = rd("A1.csv"); A2 = rd("A2.csv")              # r.jive's individual structure
        meta = rd("meta.csv")
